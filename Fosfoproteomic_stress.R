...
# Identificar las líneas que no cumplen con el mínimo de 3 datos no vacíos en tabla_stress_relax
for (n in 1:nprot) {
...
write.xlsx(significativa_stress, "datos/estadística_stress_BR16_machos.xlsx", sheetName = "Desrreguladas")
...
write.xlsx(df_desrreguladas, "datos/efecto_stress_BR16_machos.xlsx", sheetName = "Desrreguladas")
...
proteinas_all <-
  tibble(
    ...
    P_Value = df_desrreguladas$log10pvalor_stress,
    Cociente = df_desrreguladas$log2cociente_stress  ,
  )
...
sample_metadata <- read.csv("alcohol/sample-metadata.3.csv")
...
desrreguladas<-significativa_stress$Protein
